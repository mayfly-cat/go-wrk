...
type RequesterStats struct {
	TotRespSize     int64
	TotDuration     time.Duration // 成功请求（HTTP 200）的总耗时
	TotDurationAll  time.Duration // 全部请求（成功 + 失败）的总耗时（超时按实际等待时间计算）
	NumRequests     int           // 成功请求数（仅 HTTP 200）
	...
	// NetStatusCounts 按“网络状态 / HTTP 状态码”分组的请求数量统计（例如：200、404、500、error 等）
...
// DoRequest single request implementation. Returns the size of the response, its duration and HTTP status code.
// 仅当 HTTP 状态码为 200 时认为请求成功；其他状态码都作为错误返回。
// On error - duration 记录从发起请求到出错的实际耗时（例如超时会接近 timeout 设置），respSize 为 0。
func DoRequest(httpClient *http.Client, header map[string]string, method, host, loadUrl, reqBody string) (respSize int, duration time.Duration, statusCode int, err error) {
	respSize = -1
	duration = -1
	statusCode = 0
	...
	if resp == nil {
		return 0, duration, 0, errors.New("empty response")
	}
	...
	if err != nil {
		return 0, duration, statusCode, err
	}
	// 仅 HTTP 200 视为成功请求，其余状态码全部视为错误
	if resp.StatusCode == http.StatusOK {
		duration = time.Since(start)
		respSize = len(body) + int(util.EstimateHttpHeadersSize(resp.Header))
		return respSize, duration, statusCode, nil
	}

	// 非 200 的 HTTP 状态都当作错误返回，方便在上层做 netStatus 统计
	return 0, duration, statusCode, errors.New(fmt.Sprint("received status code ", resp.StatusCode))
}
...
func (cfg *LoadCfg) RunSingleLoadSession() {
	...
	for time.Since(start).Seconds() <= float64(cfg.duration) && atomic.LoadInt32(&cfg.interrupted) == 0 {
		...
		respSize, reqDur, statusCode, err := DoRequest(httpClient, cfg.header, cfg.method, cfg.host, cfg.testUrl, cfg.reqBody)
		...
		// 计算当前请求的 netStatus（优先使用 HTTP 状态码，其次使用通用错误分组）
		netStatus := "unknown"
		if statusCode > 0 {
			netStatus = fmt.Sprint(statusCode)
		} else if err != nil {
			netStatus = "error"
		}
		stats.NetStatusCounts[netStatus]++
